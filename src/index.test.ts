...
test("resolves concurrent promises in correct order", assert => {
  ...
  q.add(() => after(100, "two"))
    ...
    .finally(() => {
      assert.equal(
        q.stats().jobs.total,
        2,
        "Task two wasn't correctly removed"
      );
      assert.equal(
        q.stats().jobs.inProgress,
        2,
        "Task three wasn't immediately picked up"
      );
    });
  ...
  q.add(() => after(20, "three"))
    ...
    .finally(() => {
      ...
      assert.equal(
        q.stats().jobs.inProgress,
        1,
        "Only first task is in progress"
      );
    });
  ...
  assert.equal(
    q.stats().jobs.inProgress,
    2,
    "Queue didn't start two concurrent jobs"
  );
});
  ...
  q.add(() => rejectAfter(100, "one"))
    ...
    .finally(() => {
      ...
      assert.equal(
        rejected,
        true,
        "Queue didn't handle promise rejection properly"
      );
    });
});
