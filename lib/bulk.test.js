...
test("Bulk test", assert => {
    ...
    const promises = Array(tasks)
        ...
        const promise = queue
            ...
            if (err instanceof index_1.QueueTimeout) {
                stats.queuetimeout++;
                console.log(i, "queuetimeout");
            }
            else if (err instanceof index_1.QueueFull) {
                stats.queuefull++;
                console.log(i, "queuefull");
            }
            else if (err instanceof index_1.JobCancelled) {
                console.log(i, "jobcancelled");
                stats.cancelled++;
            }
            else if (err instanceof index_1.JobTimeout) {
                stats.timeout++;
    ...
    Promise.all(promises).finally(() => {
        ...
        assert.ok(close(fails + successes + timeouts, 1), `fails ${fails} successes ${successes} and timeouts ${timeouts} must add to 1`);
        assert.ok(close(stats.success / tasks, successes), `successes ${stats.success / tasks} must be close to ${successes}`);
        assert.ok(close(stats.fail / tasks, fails), `fails ${stats.fail / tasks} must be close to ${fails}`);
        assert.ok(close(stats.timeout / tasks, timeouts), `timeouts ${stats.timeout / tasks} must be close to ${timeouts}`);
    });
});
function close(n1, n2) {
    return Math.abs(n1 - n2) < 0.05;
}
//# sourceMappingURL=bulk.test.js.map